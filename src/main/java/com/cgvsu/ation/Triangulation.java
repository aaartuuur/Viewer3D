...
public final class Triangulation {
     ...
     */
    public static <T extends Point2f> List<int[]> earClippingTriangulate(List<T> vertices) {
     ...
     * @throws IllegalArgumentException if {@code vertices} size is less than 3
     */
    public static <T extends Point2f> List<int[]> earClippingTriangulate(List<T> vertices, List<Integer> vertexIndices) {
        int vertexIndicesCount = vertexIndices.size();
        checkVertexIndicesCount(vertexIndicesCount);
        ...
        for (boolean hasClippedEars = true; hasClippedEars;) {
            ...
            for (int i = 1; i < potentialEarsCount - 1; i++) {
                ...
                Point2f prevVertex = vertices.get(prevVertexIndex);
                Point2f curVertex = vertices.get(curVertexIndex);
                Point2f nextVertex = vertices.get(nextVertexIndex);

                float crossProduct = crossProduct(prevVertex, curVertex, nextVertex);
                // check if convex
                if ((isCCW ? crossProduct : -crossProduct) < EPS) {
                    continue;
                }
                ...
                for (int j = 0; j < vertexIndicesCount; j++) {
                    ...
                    Point2f checkedVertex = vertices.get(checkedVertexIndex);
...
//        }
     ...
     */
    private static <T extends Point2f> boolean isCounterClockwise(List<T> vertices, List<Integer> vertexIndices) {
        float area = 0;
        int vertexIndicesCount = vertexIndices.size();

        Point2f prevVertex = vertices.get(vertexIndices.get(0));
        for (int i = 1; i <= vertexIndicesCount; i++) {
            Point2f currentVertex = vertices.get(vertexIndices.get(i % vertexIndicesCount));
    ...
    static float crossProduct(Point2f a, Point2f b, Point2f c) {
        float dx1 = b.x - a.x;
        float dy1 = b.y - a.y;
        float dx2 = c.x - a.x;
        float dy2 = c.y - a.y;
    ...
    static boolean isPointInTriangle(Point2f a, Point2f b, Point2f c, Point2f p) {
